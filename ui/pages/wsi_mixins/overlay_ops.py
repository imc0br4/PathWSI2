...
from PySide6.QtWidgets import QMessageBox, QFileDialog, QGraphicsItem
...
class OverlayOpsMixin:
    ...
    # =========================
    # bbox/roi 兼容：xywh / xyxy
    # =========================
    def _normalize_bbox_level0_xywh(self, roi, w_px: int, h_px: int, ds: float):
        """
        统一返回 (x0,y0,w0,h0)。
        兼容旧格式 roi=[x0,y0,x1,y1] 或新格式 roi=[x0,y0,w0,h0]。
        利用 overlay 像素尺寸 * ds 推断哪个更像。
        """
        if not (isinstance(roi, (list, tuple)) and len(roi) == 4):
            return None

        try:
            x0, y0, a, b = [int(round(float(v))) for v in roi]
        except Exception:
            return None

        ds = float(ds) if ds else 1.0
        exp_w0 = int(round(w_px * ds))
        exp_h0 = int(round(h_px * ds))
        ...
        def score(w, h):
            if w <= 0 or h <= 0:
                return 10**18
            return abs(w - exp_w0) + abs(h - exp_h0)

        s1 = score(w1, h1)
        s2 = score(w2, h2)

        if s2 < s1:
            return (x0, y0, int(w2), int(h2))
        return (x0, y0, int(w1), int(h1))
    ...
    def _on_export_overlay_clicked(self):
        ...
        # 兼容：bbox_level0 可能是 xyxy 或 xywh
        roi_raw = meta.get("bbox_level0") or meta.get("roi_level0") or meta.get("bbox_level0_xyxy") or [int(ox), int(oy), w0, h0]
        roi_xywh = self._normalize_bbox_level0_xywh(
            roi_raw,
            w_px=int(self._overlay_rgba.shape[1]),
            h_px=int(self._overlay_rgba.shape[0]),
            ds=ds
        )
        if roi_xywh is None:
            roi_xywh = (int(ox), int(oy), int(w0), int(h0))

        patch_level = meta.get("patch_size_level", None)
        stride_level = meta.get("stride_size_level", None)
        grid_px = int(self._grid_tile_px)  # 已在加载/应用时推断过
        ...
        def _ok(out: dict):
            lines = []
            if "overlay" in out:
                lines.append(f"overlay: {out['overlay']}")
            if "overlay_meta" in out:
                lines.append(f"overlay_meta: {out['overlay_meta']}")
            if not lines:
                lines.append("（没有文件被保存）")
    ...
    def on_clear_overlay(self):
        self._set_edit_mode('off')  # 先退出编辑
        self._destroy_overlay_item()
        self._clear_overlay_state()
        if hasattr(self, "btn_edit"):
            self.btn_edit.setEnabled(False)

    def _resolve_overlay_alignment(
        self, w: int, h: int, meta: Optional[dict]
        ...
        if isinstance(meta, dict):
            ...
            roi_raw = meta.get("bbox_level0") or meta.get("roi_level0") or meta.get("bbox_level0_xyxy")
            roi_xywh = self._normalize_bbox_level0_xywh(roi_raw, w_px=w, h_px=h, ds=ds)

            if roi_xywh is None:
                w0, h0 = dims[0]
                roi_xywh = (0, 0, int(w0), int(h0))

            return lvl, ds, roi_xywh
        ...
        ds = ds_list[lvl]
        w0, h0 = dims[0]
        return lvl, ds, (0, 0, int(w0), int(h0))
